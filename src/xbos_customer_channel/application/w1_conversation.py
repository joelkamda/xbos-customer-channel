...
class W1ConversationRouter:
    """W1 navigation only: browse and interaction-cart updates, never order/payment execution."""

    def __init__(self, catalog: CatalogQuoteService) -> None:
        ...
        *,
        ...
        if action == "help":
            return W1ConversationResult(
                W1RenderedMessage(W1RenderKind.BUTTONS, "Choose Menu to browse, or Restart to return here.", (W1Button("menu", "Menu"), W1Button("restart", "Restart"))),
                catalog_session,
                navigation,
            )
        if action == "back":
        ...
        return W1ConversationResult(
            W1RenderedMessage(W1RenderKind.TEXT, "I did not understand that. Reply Menu, Help, or Restart."),
            catalog_session,
            navigation,
        )
    ...
    def _assert_server_session(session: CustomerSessionSnapshot) -> None:
        if not session.security_binding_complete or not session.session_ref or session.state not in {ChannelState.BROWSING, ChannelState.CART}:
    ...
    def _action(inbound: NormalizedInboundMessage) -> str:
        if inbound.interactive_reply is not None:
            if inbound.interactive_reply.kind not in {InteractionKind.BUTTON_REPLY, InteractionKind.LIST_REPLY}:
                return "unsupported"
    ...
    def _root(self, catalog_session: CatalogSession) -> W1ConversationResult:
        categories = tuple(
            ...
            for section in sorted(catalog_session.projection.sections, key=lambda section: section.sort_order)
        )
        return W1ConversationResult(
            W1RenderedMessage(
                W1RenderKind.LIST,
    ...
    def _section(self, catalog_session: CatalogSession, section_ref: str) -> W1ConversationResult:
        section = next((section for section in catalog_session.projection.sections if section.section_ref == section_ref), None)
        if section is None:
        ...
        if not rows:
            return self._unsupported(catalog_session)
        return W1ConversationResult(
            W1RenderedMessage(W1RenderKind.LIST, f"{section.name}: choose an item.", list_button_label="Items", list_sections=(W1ListSection(section.name, rows),)),
            catalog_session,
            W1NavigationCursor(active_section_ref=section_ref),
        )

    def _item(self, catalog_session: CatalogSession, item_ref: str) -> W1ConversationResult:
        try:
            item = catalog_session.projection.item(item_ref)
        except KeyError:
            return self._unsupported(catalog_session)
        return self._product(catalog_session, item, W1NavigationCursor(active_section_ref=item.section_ref, active_item_ref=item_ref))

    def _quantity(self, catalog_session: CatalogSession, navigation: W1NavigationCursor, value: str) -> W1ConversationResult:
        ...
        try:
            ...
            return self._unsupported(catalog_session)
        return self._product(catalog_session, item, W1NavigationCursor(navigation.active_section_ref, item.item_ref, int(value)))

    def _add(self, catalog_session: CatalogSession, navigation: W1NavigationCursor) -> W1ConversationResult:
        if navigation.active_item_ref is None:
            return self._unsupported(catalog_session)
        try:
            updated = self._catalog.add_to_cart(catalog_session, item_ref=navigation.active_item_ref, quantity=navigation.quantity)
            item = updated.projection.item(navigation.active_item_ref)
        except (KeyError, ValueError):
            return self._unsupported(catalog_session)
        return W1ConversationResult(
            W1RenderedMessage(W1RenderKind.BUTTONS, f"Added {navigation.quantity} × {item.name}. Continue browsing?", (W1Button("menu", "Menu"), W1Button("back", "Back"))),
            updated,
            navigation,
        )

    def _back(self, catalog_session: CatalogSession, navigation: W1NavigationCursor) -> W1ConversationResult:
    ...
    def _product(self, catalog_session: CatalogSession, item: CatalogItemProjection, navigation: W1NavigationCursor) -> W1ConversationResult:
        return W1ConversationResult(
            W1RenderedMessage(
                W1RenderKind.BUTTONS,
                f"{item.name}: {item.description}\nDisplayed price: {item.display_price} {item.currency}\nQuantity: {navigation.quantity}",
                (W1Button("qty:1", "1"), W1Button("qty:2", "2"), W1Button("add", "Add to cart")),
            ),
            catalog_session,
            navigation,
        )
    ...
    def _unsupported(self, catalog_session: CatalogSession) -> W1ConversationResult:
        return W1ConversationResult(
            W1RenderedMessage(W1RenderKind.TEXT, "That selection is no longer available. Reply Menu to browse safely."),
            catalog_session,
            W1NavigationCursor(),
        )
